    ...
    this.configureCommand({
      ...
      builder: (yargs) =>
        yargs
          ...
          .example(
            'echo \'{"consumer":"jdoe", "type": "key-auth"}\'' +
              `| $0 ${process.argv[2]} create --stdin`,
          )
          .example(
            'echo \'{"consumer":"jdoe", "type": "key-auth", "scopes":["existingScope"]}\'' +
              `| $0 ${process.argv[2]} create --stdin`,
          )
          .example(`cat all_apps.json | $0 ${process.argv[2]} create --stdin`)
          .example(
            `$0 ${process.argv[2]} create -u jdoe -p 'scopes=existingScope'`,
          )
          .string(["p", "c", "t"])
          .boolean(["stdin"])
          ...
          .check((args, _opts) => {
            ...
            if (!(args.t && args.c)) {
              throw new Error(
                "must include either --stdin or -c with -t (--consumer with --type)",
              );
            }
            return true;
          }),
  ...
  _createFromCommandLine() {
    ...
    return this._promptAndValidate(credential, SCHEMA)
      .then((credential) => {
        ...
        return this.admin.credentials.create(
          argv.consumer,
          argv.type,
          credential,
        );
