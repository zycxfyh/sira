...
module.exports = class extends eg.Generator {
  ...
  prompting() {
    ...
    const commandLineOptionKeys = this.pluginOptions ? Object.keys(this.pluginOptions) : [];

    commandLineOptionKeys.forEach(key => {
      const schema = optionsMeta[key];
      if (!this._validateOption(key, this.pluginOptions[key], schema)) {
        this.log.error(
          `Invalid plugin option value for ${key}, ` + `expected: ${schema.type || 'string'}`
        );
      }
    });
    ...
    return this.prompt(questions).then(answers => {
      ...
      this.addPoliciesToWhitelist = this.addPoliciesToWhitelist || answers.addPoliciesToWhitelist;
