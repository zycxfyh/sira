...
module.exports = class extends eg.Generator {
    ...
    this.configureCommand({
      ...
      builder: (yargs) =>
        yargs
          ...
          .example(
            `$0 ${process.argv[2]} install express-gateway-plugin-url-rewrite`,
          )
          .option("n", {
            alias: "enable",
  ...
  prompting() {
    ...
    const commandLineOptionKeys = this.pluginOptions
      ? Object.keys(this.pluginOptions)
      : [];
    ...
    commandLineOptionKeys.forEach((key) => {
      ...
      if (!this._validateOption(key, this.pluginOptions[key], schema)) {
        this.log.error(
          `Invalid plugin option value for ${key}, ` +
