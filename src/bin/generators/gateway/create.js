...
module.exports = class extends eg.Generator {
  ...
  prompting() {
    ...
    return Promise.resolve()
      .then(() => {
        if (!this.name) {
          return this.prompt([nameQuestion]).then((props) => {
      ...
      .then(() => {
        if (!this.directory) {
          directoryQuestion.default = path.relative(process.cwd(), this.name);

          return this.prompt([directoryQuestion]).then((props) => {
      ...
      .then(() => {
        if (!this.type) {
          return this.prompt([typeQuestion]).then((props) => {
  ...
  writing() {
    ...
    this.fs.copy(
      ...
      this.destinationPath("config/models"),
    );
  }
  ...
  install() {
    ...
    console.log("");
    console.log(
      `To start ${chalk.green(this.name)}, run the following commands:`,
    );
