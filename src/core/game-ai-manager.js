...
保持变化的合理性和沉浸感。`;
    ...
    try {
      ...
      logError(`世界状态更新失败: ${error.message}`);
      throw error;
    }
  }
  ...
  cleanup() {
    ...
    if (cleanedSessions > 0) {
      logInfo(`清理了 ${cleanedSessions} 个过期会话`);
    }

    if (cleanedMemories > 0) {
      logInfo(`清理了 ${cleanedMemories} 个过期记忆`);
    }
  }
}
