...
module.exports = function (params, config) {
  ...
  return async function aiRouter(req, res, next) {
    ...
    try {
      ...
      if (abTestAllocation) {
        const { variant } = abTestAllocation;
        logger.info(
          `A/B测试分配: ${abTestAllocation.testId} -> 变体 ${abTestAllocation.variantId}`,
          {
            userId,
            requestId,
    ...
    let selectedProvider = selectBestProvider(model, req);
    ...
    // Set routing results in egContext (for testing compatibility)
    req.egContext.targetService = selectedProvider;
    req.egContext.targetUrl = targetUrl;
