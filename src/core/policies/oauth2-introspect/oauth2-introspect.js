    ...
    new BearerStrategy(
      ...
      (req, accessToken, done) => {
        ...
        const scopeCheck = (tokenData, done) => {
          ...
          const avaiableScopes = tokenDataScopes
            ? tokenDataScopes.split(" ")
            : [];

          if (
            requestedScopes.every((scope) => avaiableScopes.includes(scope))
          ) {
        ...
        db.get(`accesstoken.${accessToken}`).then((result) => {
          if (result) {
