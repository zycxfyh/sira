...
module.exports = (app, config) => {
  ...
  if (config.systemConfig.session.storeProvider) {
    try {
      const ProviderStore = require(config.systemConfig.session.storeProvider)(
        session,
      );
      config.systemConfig.session.store = new ProviderStore(
        config.systemConfig.session.storeOptions,
      );
      delete config.systemConfig.session.storeProvider;
      delete config.systemConfig.session.storeOptions;
    } catch (error) {
      logger.error(
        `Failed to initialize custom express-session store, please ensure you have ${config.systemConfig.session.storeProvider} npm package installed`,
      );
      throw error;
    }
  }
