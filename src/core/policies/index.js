...
const register = (policyOptions) => {
  ...
  policyOptions.policy = (params, ...args) => {
    ...
    logger.error(
      `Policy ${chalk.default.red.bold(name)} params validation failed: ${validationResult.error}`,
    );
...
schemas.register("internal", "baseAuth", {
  ...
  properties: {
    passThrough: {
      ...
      default: false,
      description:
        "Specify whether continue with pipeline processing in case the authentication fails",
    },
  },
...
const load = (policiesToBeLoaded) => {
  const corePolicies = policiesToBeLoaded.filter((name) =>
    policyNames.includes(name),
  );
  ...
  // Auto-load AI policies
  const aiPolicies = policyNames.filter((name) => name.startsWith("ai-"));
  aiPolicies.forEach((name) => {
    if (!policies[name]) {
      try {
        logger.debug(`registering AI policy ${chalk.green(name)}`);
        const policy = require(path.resolve(__dirname, name));
        policies[name] = Object.assign(policy, { name });
        register(policy);
      } catch (error) {
        logger.warn(`Failed to load AI policy ${name}:`, error.message);
      }
    }
  });
};
