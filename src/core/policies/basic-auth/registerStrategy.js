...
module.exports = function registerBasicStrategy() {
  ...
  function authenticateBasic(req, clientId, clientSecret, done) {
    ...
    if (req.egContext && req.egContext.apiEndpoint && req.egContext.apiEndpoint.scopes) {
      endpointScopes =
        req.egContext.apiEndpoint.scopes && req.egContext.apiEndpoint.scopes.map(s => s.scope || s);
    ...
    return authService
      ...
      .then(consumer => {
        ...
        return authService
          .authorizeCredential(consumer.id, credentialType, endpointScopes || requestedScopes)
          .then(authorized => {
            if (!authorized) {
              return done(null, false);
            }
