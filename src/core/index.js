...
// 快速失败：验证构造函数存在
if (!DocGenerator || !KnowledgeBase || !SearchSystem || !I18nManager || !TemplateEngine) {
...
class ExtensionsManager {
  constructor(options = {}) {
    ...
    try {
      ...
      if (!this.docGenerator || typeof this.docGenerator.generate !== 'function') {
        throw new Error('DocGenerator初始化失败');
      }

      this.knowledgeBase = new KnowledgeBase();
      if (!this.knowledgeBase || typeof this.knowledgeBase.search !== 'function') {
  ...
  scaffoldProject(template, destination) {
    return this.templateEngine ? this.templateEngine.scaffold(template, destination) : Promise.reject(new Error('Template engine not available'));
  }

  generateCode(type, options) {
    return this.templateEngine ? this.templateEngine.generate(type, options) : Promise.reject(new Error('Template engine not available'));
  }
}
