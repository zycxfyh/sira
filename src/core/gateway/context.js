...
EgContextBase.prototype.safeEval = function (expression) {
  ...
  if (
    /function|eval|require|import|process|global|this\.|new|class|=>|prototype|constructor/i.test(
      expression,
    )
  ) {
  ...
  if (!allowedPattern.test(expression.trim())) {
    throw new Error(
      "Unsafe expression: only property access and basic comparisons allowed",
    );
  }
...
EgContextBase.prototype.evaluateAsTemplateString = (_expression) => {
  // Template strings are inherently unsafe - disable
  throw new Error(
    "Template string evaluation is disabled for security reasons",
  );
};
...
EgContextBase.prototype.run = (_code, _ctx) => {
  // SECURITY CRITICAL: vm.runInNewContext is UNSAFE and allows RCE
  // This method has been disabled for security reasons
  throw new Error(
    "Arbitrary code execution is disabled for security reasons. Use safe expression evaluation instead.",
  );
};
