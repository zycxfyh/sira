...
module.exports = ({ plugins, config } = {}) => {
  ...
  if (cfg.admin.hostname) {
    logger.warn(
      "Warning! use of hostname is deprecated in admin section, use host instead",
    );
  }
  ...
  app.use(
    "/service-endpoints",
    require("./routes/service-endpoints")({ config }),
  );
  ...
  app.use("/", require("./routes/analytics")({ logger }));
  app.use("/", require("./routes/parameters")({ logger }));
  app.use("/", require("./routes/prompt-templates")({ logger }));
  app.use("/", require("./routes/benchmark")({ logger }));
  app.use("/", require("./routes/api-keys")({ logger }));
