...
module.exports = function ({ logger }) {
  ...
  router.get('/prompt-templates', async (req, res) => {
    try {
      ...
      if (category) {
        ...
        result = {
          search,
          recommendations: promptTemplateManager.getRecommendedTemplates(search, 10)
        }
      } else {
        // 获取所有模板
        result = {
   ...
   * 获取所有模板分类
   */
  router.get('/prompt-templates/categories', async (req, res) => {
    try {
  ...
  router.post('/prompt-templates/render', async (req, res) => {
    try {
      ...
      // 渲染模板
      const result = promptTemplateManager.renderTemplate(category, templateId, variables || {})
  ...
  router.post('/prompt-templates/recommend', async (req, res) => {
    try {
      ...
      // 获取推荐模板
      const recommendations = promptTemplateManager.getRecommendedTemplates(taskDescription, limit)
  ...
  router.post('/prompt-templates/custom', async (req, res) => {
    try {
      ...
      // 添加自定义模板
      const success = promptTemplateManager.addCustomTemplate(category, templateId, {
        ...template,
   ...
   * 删除自定义模板
   */
  router.delete('/prompt-templates/custom/:category/:templateId', async (req, res) => {
    try {
      const { category, templateId } = req.params

      // 删除自定义模板
      const success = promptTemplateManager.removeCustomTemplate(category, templateId)
   ...
   * 获取模板使用统计
   */
  router.get('/prompt-templates/stats', async (req, res) => {
    try {
