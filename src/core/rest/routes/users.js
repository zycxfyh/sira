...
module.exports = () => {
  ...
  router.get("/:id", (req, res, next) => {
    usersSrv
      .findByUsernameOrId(req.params.id)
      .then((user) => {
        if (!user)
  ...
  router.delete("/:id", (req, res, next) => {
    usersSrv
      ...
      .then((user) => {
        ...
        return usersSrv.remove(user.id).then(() => {
          res.status(204).send();
        });
      })
      .catch(next);
    ...
    usersSrv
      ...
      .then((user) => {
        ...
        if (req.body.status === true) {
          return usersSrv.activate(user.id).then((_status) => {
            res.json({ status: "Activated" });
          });
        } else {
          return usersSrv.deactivate(user.id).then((_status) => {
