...
function modelTrainingRoutes() {
  ...
  router.delete("/datasets/:datasetId", async (req, res) => {
    try {
      ...
      // 检查是否有训练作业使用此数据集
      for (const [_jobId, job] of modelTrainingManager.trainingJobs) {
        if (
      ...
      // 删除实际文件
      try {
        await require("node:fs").promises.unlink(dataset.filePath);
      } catch (fileError) {
  ...
  router.post("/jobs", async (req, res) => {
    try {
      ...
      // 设置用户ID
      jobConfig.userId =
        jobConfig.userId || req.headers["x-user-id"] || "anonymous";
  ...
  router.post("/jobs/:jobId/deploy", async (req, res) => {
    try {
      ...
      const deployedModel = await modelTrainingManager.deployTrainedModel(
        jobId,
        deploymentConfig,
      );
  ...
  router.get("/stats", async (_req, res) => {
    try {
      ...
      const stats = {
        ...
        activeModels: models.filter((model) => model.status === "deployed")
          .length,
...
// 辅助函数：从buffer创建可读流
function _createReadStreamFromBuffer(buffer) {
