...
class PerformanceBenchmarkManager extends EventEmitter {
  ...
  analyzePerformance(results) {
    ...
    if (Object.keys(modelTimes).length > 0) {
      ...
      analysis.slowest_model = {
        model: sorted[sorted.length - 1][0],
        time: sorted[sorted.length - 1][1],
      };
  ...
  analyzeCost(results) {
    ...
    if (Object.keys(modelCosts).length > 0) {
      ...
      analysis.most_expensive_model = {
        model: sorted[sorted.length - 1][0],
        cost: sorted[sorted.length - 1][1],
      };
  ...
  generateRecommendations(results, config) {
    ...
    for (const [model, scores] of Object.entries(modelScores)) {
      ...
      modelScores[model].overall_score =
        (normalizedScores.speed_score +
          normalizedScores.cost_score +
          normalizedScores.quality_score) /
        3;
    }
  ...
  async runSingleTest(model, _task, testCase, _iteration, config, _testId) {
    ...
    try {
      ...
      const qualityScore = config.includeQualityAssessment
        ? this.assessQuality(mockResponse.content, testCase.expected_output)
        : null;
