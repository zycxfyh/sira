...
class RulesEngine {
  ...
  async initialize () {
    ...
    try {
      ...
      console.log(`✅ 规则引擎已初始化，加载了 ${this.rules.size} 个规则和 ${this.ruleSets.size} 个规则集`)
    } catch (error) {
  ...
  getRuleStats (ruleId = null) {
    if (ruleId) {
      ...
      return {
        ...
        successRate: rule.executionCount > 0 ? (rule.successCount / rule.executionCount * 100).toFixed(2) : 0,
  ...
  async executeWebhookAction (params, context, matchResult) {
    ...
    if (global.webhookManager) {
      await global.webhookManager.triggerEvent('rule.executed', {
        ruleId: matchResult.metadata.ruleId,
  ...
  applyTransform (value, transform) {
    ...
    switch (type) {
      ...
      case 'replace':
        return typeof value === 'string' ? value.replace(new RegExp(params.pattern, 'g'), params.replacement || '') : value
      case 'multiply':
