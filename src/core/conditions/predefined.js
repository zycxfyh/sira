...
module.exports = [
  ...
  },
  {
    name: "pathMatch",
    handler: (config) => (req) =>
      req.url.match(new RegExp(config.pattern)) !== null,
    schema: {
    ...
    schema: {
      ...
      properties: {
        pattern: {
          type: "string",
          description:
            "Pattern to match against the HOST header in the current HTTP Request",
        },
      },
      required: ["pattern"],
    },
    ...
    schema: {
      ...
      properties: {
        expression: {
          type: "string",
          description:
            "Javascript Expression to execute with the current egContext object",
        },
      },
      required: ["expression"],
    },
