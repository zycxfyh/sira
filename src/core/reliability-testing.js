...
class ReliabilityTestingTool extends EventEmitter {
  ...
  async runBasicHealthTest (duration) {
    ...
    const checkInterval = this.options.checkInterval
  ...
  async runNetworkStabilityTest (duration) {
    ...
    while (Date.now() < endTime && this.isRunning) {
      ...
      if (latencyResult.latency > 5000) { // 5秒
  ...
  recordOutage (type, details) {
    if (this.testStats.currentOutage) {
      ...
      this.testStats.currentOutage.duration = this.testStats.currentOutage.endTime - this.testStats.currentOutage.startTime
  ...
  generateReliabilityReport () {
    ...
    return {
      ...
      recommendations: this.generateReliabilityRecommendations(uptime, errorRate, avgResponseTime, sloStatus)
    }
  }
  ...
  generateReliabilityRecommendations (uptime, errorRate, avgResponseTime, sloStatus) {
    ...
    if (uptime < this.options.uptimeTarget) {
      recommendations.push(`可用性未达到目标 ${this.options.uptimeTarget}%，当前: ${uptime.toFixed(2)}%。建议加强系统稳定性。`)
    }

    if (errorRate > this.options.errorRateTarget) {
      recommendations.push(`错误率超过目标 ${this.options.errorRateTarget}%，当前: ${errorRate.toFixed(2)}%。建议改进错误处理。`)
    }

    if (avgResponseTime > this.options.responseTimeTarget) {
      recommendations.push(`平均响应时间超过目标 ${this.options.responseTimeTarget}ms，当前: ${avgResponseTime.toFixed(2)}ms。建议优化性能。`)
    }
...
class RecoveryTester {
  ...
  getStats () {
    ...
    return {
      ...
      passed,
      successRate: this.recoveryTests.length > 0 ? (passed / this.recoveryTests.length * 100).toFixed(2) : 0
    }
  }
}
