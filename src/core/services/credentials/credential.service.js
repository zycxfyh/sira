...
const dereferencePromise = refParser
  .dereference(config.models.credentials)
  .then((derefSchema) => mergeAllOf(derefSchema));

s.insertScopes = (scopes) =>
  validateNewScopes(scopes).then((newScopes) => {
    if (!newScopes) {
...
s.removeScopes = (scopes) =>
  credentialDao.removeScopes(scopes).then((v) => !!v);

s.existsScope = (scope) =>
  scope ? credentialDao.existsScope(scope) : Promise.resolve(false);

s.getAllScopes = () => credentialDao.getAllScopes();
...
s.insertCredential = function (id, type, credentialDetails) {
  ...
  const checkSingleCredExistence = () => {
    return this.getCredential(id, type).then((cred) => {
      if (cred?.isActive) {
  ...
  return flow.then((resolvedSchema) => {
      ...
      return Promise.all([
        ...
        .then(([scopes, credentialProps]) => {
          ...
          return Promise.all([
            ...
            credentialDao.associateCredentialWithScopes(
              newCredential.keyId,
              type,
              scopes,
            ),
    ...
    return Promise.all([
      ...
      .then(([password]) => {
        ...
        if (password) {
          credential[credentialConfig.properties.passwordKey.default] =
            password;
        }
...
s.getCredentials = (consumerId, _options) =>
  credentialDao
...
s.addScopesToCredential = function (id, type, scopes) {
  return Promise.all([
    ...
    const existingScopes = credential.scopes
    ...
    return Promise.all([
      credentialDao.updateCredential(id, type, {
...
s.removeScopesFromCredential = function (id, type, scopes) {
  return this.getCredential(id, type).then((credential) => {
    if (!credential) {
      throw new Error("Credential not found");
    }

    const newScopes = credential.scopes.filter(
      (val) => scopes.indexOf(val) === -1,
    );
    return Promise.all([
      credentialDao.updateCredential(id, type, {
        scopes: JSON.stringify(newScopes),
...
s.setScopesForCredential = function (id, type, scopes) {
  return this.getCredential(id, type)
    .then((credential) => {
      ...
      return credentialDao.updateCredential(id, type, {
...
function processCredential(credential, options = { includePassword: false }) {
  ...
  if (
    ...
    credentialModel.properties &&
    credentialModel.properties.passwordKey
  ) {
...
function validateAndHashPassword(credentialConfig, credentialDetails) {
  if (credentialDetails[credentialConfig.properties.passwordKey.default]) {
    return utils
      .saltAndHash(
        credentialDetails[credentialConfig.properties.passwordKey.default],
      )
      .then((hash) => ({ hash }));
  }
  ...
  if (!credentialConfig.properties.autoGeneratePassword.default) {
    throw new Error(
      `${credentialConfig.properties.passwordKey.default} is required`,
    ); // TODO: replace with validation error
  }
  const password = uuidv4();

  return utils.saltAndHash(password).then((hash) => ({ hash, password }));
}
...
function validateNewCredentialScopes(credentialConfig, credentialDetails) {
  ...
  if (credentialConfig.required?.includes("scopes")) {
...
function validateUpdatedCredentialProperties(type, credentialDetails) {
  ...
  return Object.keys(newCredentialProperties).length > 0
    ? Promise.resolve(newCredentialProperties)
    : Promise.resolve(null);
}

function validateNewScopes(scopes) {
  return grabScopesAndExecute(scopes, (val) => !val).catch(() => {
    throw new Error("One or more scopes already exist.");
  });
}
...
function grabScopesAndExecute(scopes, fn) {
  return Promise.all(scopes.map(s.existsScope)).then((res) => {
    if (res.every(fn)) {
      return scopes;
    }
