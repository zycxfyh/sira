...
dao.find = function (appName) {
  return db.smembers(appNameSetKey(appName)).then(Ids => {
    if (Ids && Ids.length !== 0) {
      if (Ids.length === 1) {
        return Ids[0];
      }
...
dao.get = function (id) {
  return db.hgetall(appHashKey(id)).then(app => {
...
dao.getAll = function (userId) {
  return this.getAllAppIdsByUser(userId).then(appIds => {
    return Promise.all(appIds.map(this.get)).then(apps => apps.filter(app => app !== false));
  });
};
...
dao.removeAll = function (userId) {
  return this.getAllAppIdsByUser(userId).then(appIds => {
    ...
    return Promise.all(removeAppPromises).then(responses => responses.every(res => res));
  });
};
