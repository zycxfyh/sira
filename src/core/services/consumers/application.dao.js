...
const appHashKey = (value) =>
  `${config.systemConfig.db.redis.namespace}-${appNamespace}:${value}`;

// key for the user-applications hash table
const userAppsHashKey = (value) =>
  `${config.systemConfig.db.redis.namespace}-${userAppsNamespace}:${value}`;

// key for application name-id hash table
const appNameSetKey = (value) =>
  `${config.systemConfig.db.redis.namespace}-${appnameNamespace}:${value}`;
    ...
    if (appId) {
      return dao.get(appId).then((possibleApp) => {
        if (possibleApp.userId === app.userId) {
          throw new Error(
            `${app.userId} has already another application bound with ${app.name} as name`,
          );
        }
...
dao.update = (id, props) => {
  ...
  return db.hmset(hashKey, props).then((res) => {
    return !!res;
  });
};
...
dao.findAll = ({ start = 0, count = "100" } = {}) => {
  ...
  return db.scan(start, "MATCH", `${key}*`, "COUNT", count).then((resp) => {
    ...
    if (!appKeys || appKeys.length === 0)
      return Promise.resolve({ apps: [], nextKey: 0 });
...
dao.find = (appName) =>
  db.smembers(appNameSetKey(appName)).then((Ids) => {
    if (Ids && Ids.length !== 0) {
      if (Ids.length === 1) {
        return Ids[0];
      }
...
dao.get = (id) =>
  db.hgetall(appHashKey(id)).then((app) => {
...
dao.getAll = function (userId) {
  return this.getAllAppIdsByUser(userId).then((appIds) => {
    return Promise.all(appIds.map(this.get)).then((apps) =>
      apps.filter((app) => app !== false),
    );
  });
};

dao.getAllAppIdsByUser = (userId) => db.smembers(userAppsHashKey(userId));

dao.activate = (id) =>
  db.hmset(appHashKey(id), { isActive: "true", updatedAt: String(new Date()) });

dao.deactivate = (id) =>
  db.hmset(appHashKey(id), {
    isActive: "false",
...
dao.removeAll = function (userId) {
  return this.getAllAppIdsByUser(userId).then((appIds) => {
    ...
    return Promise.all(removeAppPromises).then((responses) =>
      responses.every((res) => res),
    );
  });
};
