...
s.get = (id) =>
  applicationDao.get(id).then((app) => {
    if (!app) {
      return false;
    }
...
s.find = function (appName) {
  ...
  return applicationDao.find(appName).then((app) => {
    return app ? this.get(app) : false;
  });
};

s.findAll = (query) =>
  applicationDao.findAll(query).then((data) => {
    data.apps = data.apps || [];
    data.apps.forEach((a) => {
...
s.findByNameOrId = (value) =>
  s.get(value).then((application) => {
    if (application) {
      return application;
    }
...
s.getAll = (userId) =>
  applicationDao.getAll(userId).then((apps) => {
    return apps.map((app) => {
      app.isActive = app.isActive === "true";
      return app;
    });
...
s.deactivateAll = (userId) =>
  applicationDao
    .deactivateAll(userId)
    .then(() => true)
    .catch(() =>
      Promise.reject(new Error("failed to deactivate all applications")),
    );
...
s.removeAll = (userId) =>
  applicationDao.removeAll(userId).then((removed) => {
    return removed
      ? true
      : Promise.reject(new Error("failed to remove all apps")); // TODO: replace with server error
  });
...
s.update = function (id, applicationProperties) {
  ...
  return this.get(id) // validate app exists
    .then(() => {
      if (
        !Object.keys(applicationProperties).every(
          (key) =>
            typeof key === "string" &&
            config.models.applications.properties[key],
        )
      ) {
...
function validateAndCreateApp(appProperties, userId) {
  if (!appProperties || !userId) {
    throw new Error(
      "Failed to insert application: invalid application properties",
  ...
  return userService.get(userId).then((user) => {
    if (!user) {
      throw new Error(`User ${userId} not found`);
    }

    if (
      !Object.keys(appProperties).every(
        (key) =>
          typeof key === "string" &&
          !!config.models.applications.properties[key],
      )
      ...
      throw new Error(
        "Failed to insert application: one or more property is invalid",
      ); // TODO: replace with validation error
    }
