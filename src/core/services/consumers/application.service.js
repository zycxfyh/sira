...
s.get = function (id) {
  return applicationDao.get(id).then(app => {
    if (!app) {
      return false;
    }
...
s.findByNameOrId = function (value) {
  return s.get(value).then(application => {
    if (application) {
      return application;
    }
...
s.getAll = function (userId) {
  return applicationDao.getAll(userId).then(apps => {
    return apps.map(app => {
      app.isActive = app.isActive === 'true';
      return app;
    });
...
s.removeAll = function (userId) {
  return applicationDao.removeAll(userId).then(removed => {
    ...
    .then(() => {
      if (
        !Object.keys(applicationProperties).every(
          key => typeof key === 'string' && config.models.applications.properties[key]
        )
      ) {
