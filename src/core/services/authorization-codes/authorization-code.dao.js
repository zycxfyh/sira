...
dao.find = function (criteria) {
  return db
    .hgetall(
      config.systemConfig.db.redis.namespace.concat('-', authCodeNamespace).concat(':', criteria.id)
    )
    .then(code => {
...
dao.get = function (id) {
  return db.hgetall(
    config.systemConfig.db.redis.namespace.concat('-', authCodeNamespace).concat(':', id)
  );
};
