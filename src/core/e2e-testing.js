...
class E2ETestingTool extends EventEmitter {
    ...
    try {
      ...
      if (parallel) {
        ...
        for (const journeyName of journeys) {
          const result = await this.runUserJourney(
            journeyName,
            retries,
            timeout,
          );
  ...
  async runUserJourney(
    ...
    for (let attempt = 1; attempt <= retries + 1; attempt++) {
      try {
        console.log(
          `📋 执行用户旅程: ${journey.name} (尝试 ${attempt}/${retries + 1})`,
        );
        ...
        console.warn(
          `⚠️ 用户旅程失败: ${journeyName} (尝试 ${attempt}) - ${error.message}`,
        );
        lastError = error;
  ...
  async waitForAIResponse(page) {
    await page.waitForSelector(".ai-response, .bot-message", {
      timeout: 30000,
    });
  }
  ...
  async testAPIKeyConnection(page) {
    ...
    await page.waitForSelector(".connection-success, .test-passed", {
      timeout: 10000,
    });
  }
  ...
  async verifyBatchResults(page) {
    await page.waitForSelector(".batch-completed, .results-ready", {
      timeout: 30000,
    });
  ...
  async observeStreamingResponse(page) {
    await page.waitForSelector(".streaming-response, .stream-output", {
      timeout: 10000,
    });
    // 验证流式响应是否实时更新
  ...
  async checkPerformanceMetrics(page) {
    const metricsElement = await page.$(
  ...
  generateE2ESummary(results) {
    ...
    const successfulSteps = results.reduce(
      (sum, r) => sum + (r.steps?.filter((s) => s.success).length || 0),
      0,
    );
  ...
  generateE2ERecommendations(results) {
    ...
    if (failedJourneys.length > 0) {
      recommendations.push(
        `${failedJourneys.length} 个用户旅程失败，需要修复相关功能`,
      );
    ...
    if (slowJourneys.length > 0) {
      recommendations.push(
        `${slowJourneys.length} 个旅程执行过慢，需要优化性能`,
      );
    }
