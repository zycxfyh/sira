...
const testCases = {
  ...
  simple_qa: {
    ...
    expected_output: '人工智能是计算机科学的一个分支，旨在创造能够模拟人类智能的机器。',
  ...
  text_summarization: {
    ...
    category: '文本处理',
    difficulty: '中等',
    input: '请为以下文章生成一个50字以内的摘要：人工智能的发展经历了从专家系统到机器学习的转变。近年来，深度学习技术取得了突破性进展，使得AI在图像识别、自然语言处理等领域取得了显著进步。然而，AI的发展也面临着数据隐私、算法偏见等挑战。未来，AI将更加注重可解释性和安全性。',
  ...
  translation: {
    ...
    category: '多语言任务',
    difficulty: '中等',
    input: "请将以下英文句子翻译成中文：'The rapid advancement of artificial intelligence is transforming industries across the globe.'",
  ...
  logical_reasoning: {
    ...
    category: '推理任务',
    difficulty: '困难',
    input: '所有玫瑰都是花。有些花会凋谢。因此，玫瑰会凋谢。这个推理正确吗？为什么？',
  ...
  context_understanding: {
    ...
    category: '理解任务',
    difficulty: '中等',
    input: "在软件开发项目中，'敏捷开发'和'瀑布模型'有什么区别？请从项目管理、风险控制和交付方式等方面进行比较。",
  ...
  ethical_reasoning: {
    ...
    category: '伦理任务',
    difficulty: '困难',
    input: '如果一个AI系统可以预测犯罪，但预测准确率只有70%，政府是否应该使用它来预防犯罪？请从隐私权、公平性和潜在滥用等方面分析。',
  ...
  conversational: {
    ...
    category: '对话任务',
    difficulty: '中等',
    input: '你好！我是小明，很高兴认识你。你可以自我介绍一下，然后问我一个问题吗？',
  ...
  complex_problem_solving: {
    ...
    category: '问题解决',
    difficulty: '困难',
    input: '一家小型电商公司的月销售额下降了20%。请分析可能的原因，并提出具体的改进建议。分析应该包括数据收集、根本原因分析和行动计划。',
  ...
  knowledge_integration: {
    ...
    category: '知识任务',
    difficulty: '困难',
    input: '将机器学习、物联网和可持续发展三个概念结合起来，设计一个智能城市项目。描述项目目标、技术架构和预期影响。',
  ...
  personalized_recommendation: {
    ...
    category: '推荐任务',
    difficulty: '中等',
    input: '我是一个大学生，对编程和人工智能感兴趣，但数学基础比较弱。请推荐适合我的学习路径和资源。',
  ...
  creative_solution: {
    ...
    category: '创新任务',
    difficulty: '困难',
    input: '设计一个系统，帮助人们减少食物浪费。从技术实现、用户激励和社会影响等方面进行完整设计。',
...
const dynamicTestCases = {
  ...
  vocabulary_test: {
    ...
    generateInput: () => {
      const words = [
        'serendipity', 'ephemeral', 'ubiquitous', 'pragmatic',
        'eloquent', 'resilient', 'meticulous', 'paradigm'
      ]
  ...
  random_story: {
    ...
    difficulty: '中等',
    generateInput: () => {
      const themes = ['太空探索', '时间旅行', '魔法世界', '未来科技', '海洋冒险']
      const characters = ['年轻科学家', '勇敢探险家', '神秘魔法师', '聪明机器人', '普通上班族']
      const settings = ['古老城堡', '未来城市', '深海潜艇', '太空飞船', '魔法森林']
...
const testCategories = {
  ...
  analytical: ['text_summarization', 'logical_reasoning', 'context_understanding'],
  advanced: ['ethical_reasoning', 'complex_problem_solving', 'knowledge_integration'],
...
const difficultyLevels = {
  simple: ['simple_qa', 'sentiment_analysis', 'math_calculation', 'vocabulary_test'],
  medium: ['creative_writing', 'code_generation', 'text_summarization', 'translation', 'context_understanding', 'conversational', 'real_time_response', 'personalized_recommendation', 'random_story'],
  hard: ['logical_reasoning', 'expert_knowledge', 'ethical_reasoning', 'complex_problem_solving', 'knowledge_integration', 'creative_solution']
}
...
const testSuites = {
  ...
  comprehensive_test: {
    ...
    tasks: ['simple_qa', 'creative_writing', 'code_generation', 'text_summarization', 'logical_reasoning'],
    iterations: 5,
    concurrency: 3
  },
    ...
    for (const [taskId, testCase] of Object.entries(testCases)) {
      if (
        testCase.name.toLowerCase().includes(searchTerm) ||
                ...
                (testCase.tags && testCase.tags.some(tag => tag.toLowerCase().includes(searchTerm)))
      ) {
