...
class StreamingManager extends EventEmitter {
  ...
  getConnectionStats () {
    ...
    for (const conn of connections) {
      ...
      // 最老和最新的连接
      if (!stats.oldestConnection || age > (now - new Date(stats.oldestConnection.createdAt).getTime())) {
      ...
      if (!stats.newestConnection || age < (now - new Date(stats.newestConnection.createdAt).getTime())) {
    ...
    stats.avgConnectionAge = connections.length > 0 ? Math.round(totalAge / connections.length / 1000) : 0
  ...
  getStreamStats () {
    ...
    for (const stream of streams) {
      ...
      // 最活跃的流
      if (!stats.mostActiveStream || (stream.messageCount || 0) > (stats.mostActiveStream.messageCount || 0)) {
    ...
    stats.avgConnectionsPerStream = streams.length > 0 ? (totalConnections / streams.length).toFixed(2) : 0
    stats.avgMessagesPerStream = streams.length > 0 ? (totalMessages / streams.length).toFixed(2) : 0
