...
class MultilingualManager extends EventEmitter {
  constructor(options = {}) {
    ...
    this.configPath =
      options.configPath || path.join(__dirname, "../config/multilingual.json");
    this.localesPath =
      options.localesPath || path.join(__dirname, "../locales");
    this.cachePath =
      options.cachePath || path.join(__dirname, "../cache/translations");
  ...
  async initialize() {
    try {
      ...
      console.log(
        `✅ 多语言管理器已初始化，支持 ${Object.keys(this.supportedLanguages).length} 种语言`,
      );
    } catch (error) {
  ...
  detectLanguage(request, context = {}) {
    ...
    const headerLanguage =
      request.headers["x-language"] || request.headers["x-lang"];
  ...
  async localizeResponse(response, targetLanguage, context = {}) {
    ...
    if (response.error || response.success === false) {
      ...
      if (response.error) {
        localizedResponse.error = await this.translateError(
          response.error,
          targetLanguage,
        );
      }
      ...
      if (response.message) {
        localizedResponse.message = await this.translate(
          response.message,
          "en-US",
          targetLanguage,
        );
    ...
    // 对于成功响应，根据响应结构进行本地化
    const localizedResponse = await this.localizeObject(
      response,
      targetLanguage,
      context,
    );
   ...
   * 获取翻译统计
   */
  getTranslationStatistics(_timeRange = "1h") {
    ...
    // 计算API调用率
    stats.apiCallRate =
      stats.totalRequests > 0 ? stats.apiCalls / stats.totalRequests : 0;
   ...
   * 计算检测置信度
   */
  calculateDetectionConfidence(_language, request, context) {
   ...
   */
  async translateWithGoogle(text, fromLanguage, toLanguage, _options) {
    ...
    if (!response.ok) {
      throw new Error(
        `Google Translate API error: ${data.error?.message || "Unknown error"}`,
      );
    }
   ...
   */
  async translateWithAzure(text, fromLanguage, toLanguage, _options) {
   ...
   * 使用OpenAI GPT翻译
   */
  async translateWithOpenAI(text, fromLanguage, toLanguage, _options) {
    ...
    const fromLangName =
      this.supportedLanguages[fromLanguage]?.nativeName || fromLanguage;
    const toLangName =
      this.supportedLanguages[toLanguage]?.nativeName || toLanguage;
  ...
  async localizeObject(obj, targetLanguage, context) {
    ...
    for (const [key, value] of Object.entries(localized)) {
      if (typeof value === "string") {
        ...
        localized[key] = await this.localizeObject(
          value,
          targetLanguage,
          context,
        );
  ...
  async translateError(error, targetLanguage) {
    ...
    if (typeof error === "object" && error.message) {
      ...
      localizedError.message = await this.translate(
        error.message,
        "en-US",
        targetLanguage,
      );
  ...
  updateTranslationStats(responseTime) {
    ...
    this.translationStats.avgResponseTime =
      this.translationStats.avgResponseTime * (1 - alpha) +
      responseTime * alpha;
