...
describe("REST: Applications", () => {
  ...
  describe("Insert two applications with the same name under the same user", () => {
    before(() =>
      adminHelper.admin.apps.create(username, {
        name: "appy1",
  ...
  describe("Insert an application with an user that does not exist", () => {
    it("should return an error on the second attemp", () =>
      should(
        adminHelper.admin.apps.create("IDoNotExist", {
          name: "appy1",
          redirectUri: "http://localhost:3000/cb",
        }),
    ...
    before(() =>
      Promise.all(
        Array(100)
          .fill()
          .map((_e, index) =>
            adminHelper.admin.apps.create(username, { name: index }),
          ),
      ),
    );

    it("should return a numeric value for nextKey", () =>
      adminHelper.admin.apps
        .list()
        .then((data) =>
          should(data).have.property("nextKey").Number().not.eql(0),
        ));

    it("should respect the start parameter", () =>
      adminHelper.admin.apps
        .list({ start: 20 })
        .then((data) => should(data.apps[0].name).not.eql("appy1")));

    it("should respect a count parameter", () =>
      adminHelper.admin.apps
        .list({ count: 3 })
        .then((data) => should(data.apps.length).lessThanOrEqual(3)));
  });
});
