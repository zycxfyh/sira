...
describe('API Contract Tests', () => {
    ...
    const chatResponseSchema = Joi.object({
      ...
      choices: Joi.array().items(
        Joi.object({
          ...
          finish_reason: Joi.string().valid('stop', 'length', 'content_filter').required()
        })
      ).min(1).required(),
      usage: Joi.object({
  ...
  describe('Admin API Contracts', () => {
    const apiKeySchema = Joi.object({
      ...
      key: Joi.string().pattern(/^sk_[a-zA-Z0-9]{48,}$/).required(),
    ...
    it('should validate API key creation contract', async () => {
      ...
      // 验证响应符合合同
      const { error } = apiKeySchema.validate(response.body)
      expect(error).toBeUndefined()
    })
