...
const { init, conditions } = require('../../src/core/conditions');
...
describe('conditions', () => {
  ...
  describe('allOf', () => {
    ...
    it('should return true if all of the arguments is true', () => {
      should(
        conditions.allOf({ conditions: [{ name: 'always' }, { name: 'always' }] })(req)
      ).be.true();
    });
    it('should return false if one of the arguments is false', () => {
      should(
        conditions.allOf({ conditions: [{ name: 'always' }, { name: 'never' }] })(req)
      ).be.false();
    });
  });
  ...
  describe('oneOf', () => {
    ...
    it('should return true if one of the arguments is true', () => {
      should(
        conditions.oneOf({ conditions: [{ name: 'never' }, { name: 'always' }] })(req)
      ).be.true();
    });
    it('should return true if more than one of the arguments is true', () => {
      should(
        conditions.oneOf({ conditions: [{ name: 'always' }, { name: 'always' }] })(req)
      ).be.true();
    });
    it('should return false if none of the arguments are true', () => {
      should(
        conditions.oneOf({ conditions: [{ name: 'never' }, { name: 'never' }] })(req)
      ).be.false();
    });
  });
      ...
      const rule = {
        ...
        conditions: [
          ...
          },
          {
            ...
            condition: {
              name: 'oneOf',
              conditions: [control, { name: 'pathExact', path: '/path/path/path' }],
            },
          },
        ],
      };
