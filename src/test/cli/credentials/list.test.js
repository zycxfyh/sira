...
describe('eg credentials list', () => {
  ...
  const createCredential = (username, type, options = {}, isActive = true) => {
    return adminHelper.admin.credentials.create(username, type, options).then(credential => {
  ...
  before(() => {
    ...
    return adminHelper
      ...
      .then(user => adminHelper.admin.credentials.create(user.username, 'key-auth', {}));
  });
