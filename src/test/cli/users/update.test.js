...
describe('eg users update', () => {
  ...
  it('updates a user from prompts by username', done => {
    env.hijack(namespace, generator => {
      ...
      generator.once('end', () => {
        return adminHelper.admin.users.info(user.username).then(user => {
  ...
  it('updates a user from properties by username', done => {
    ...
    env.argv = program.parse(
      'users update ' + user.username + ' -p "firstname=C1" -p "lastname=D1"'
    );
  });
  ...
  it('updates a user from properties by user ID', done => {
    env.hijack(namespace, generator => {
      ...
      generator.once('end', () => {
        return adminHelper.admin.users.info(user.id).then(user => {
    ...
    env.argv = program.parse(`users update ${user.id} ` + '-p "firstname=T1" -p "lastname=T2"');
  });
  ...
  it('prints only the user id when using the --quiet flag', done => {
    ...
    env.argv = program.parse('users update ' + user.id + ' -p "firstname=H1" -p "lastname=H2" -q');
  });
  ...
  it('errors on unknown user', done => {
    ...
    env.argv = program.parse(
      'users update asdf ' + '-p "firstname=FirstName" -p "lastname=LastName"'
    );
  });
  ...
  it('prints an error on invalid property syntax', done => {
    ...
    env.argv = program.parse(
      'users update lala -p "username=" ' + '-p "firstname=FirstName" -p "lastname=LastName"'
    );
  });
});
