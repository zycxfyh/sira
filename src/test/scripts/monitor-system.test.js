...
if (!hasBash) {
  ...
  console.warn(
    'Skipping monitor-system script tests: "bash" command not available in this environment.',
  );
}
...
describeIfBash("Monitor System Script Tests", () => {
  ...
  test("should validate configuration", async () => {
    ...
    const { code: code1 } = await runCommand(
      "bash",
      [MONITOR_SCRIPT, "check"],
      {
        env: {
    ...
    const { code: code2 } = await runCommand(
      "bash",
      [MONITOR_SCRIPT, "check"],
      {
        env: {
  ...
  test("should handle invalid command", async () => {
    const { code } = await runCommand(
      "bash",
      [MONITOR_SCRIPT, "invalid-command"],
      {
        env: {
          LOG_FILE: testLogFile,
          METRICS_FILE: testMetricsFile,
    ...
    return new Promise((resolve, reject) => {
      ...
      child.on("close", (code) => {
        ...
        expect(output).toContain(
          "System metrics functions executed successfully",
        );
        resolve();
      });
