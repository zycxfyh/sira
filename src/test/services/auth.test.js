...
describe("Auth tests", () => {
  ...
  describe("Credential Auth", () => {
    ...
    test("should not authenticate invalid user with credentials", () => {
      return authService
    ...
    test("should not authenticate valid user with invalid credentials", () => {
      return authService
    ...
    test("should authorize Credential with scopes", () => {
      return authService
    ...
    test("should not authorize Credential with invalid scopes", () => {
      return authService
        .authorizeCredential(userFromDb.id, "oauth2", [
          "otherScope",
          "someScope2",
    ...
    test("should not authorize Credential that is inActive", () => {
  ...
  describe("Token Auth", () => {
    ...
    beforeAll(async () => {
      ...
      return tokenService.save(tokenObj).then((token) => {
    ...
    test("should authenticate token", () => {
      return authService
        .authenticateToken(userAccessToken, "oauth2")
        .then((authResponse) => {
          const expectedTokenProps = [
            "consumerId",
            "expiresAt",
            "id",
            "scopes",
    ...
    test("should not authenticate invalid token", () => {
      authService
        .authenticateToken("invalidToken", "oauth2")
        .then((authResponse) => {
          should.exist(authResponse);
          authResponse.should.eql(false);
        });
    ...
    test("should authorize Token", () => {
      authService
        .authorizeToken(userAccessToken, "oauth2", ["someScope1", "someScope2"])
        .then((authResponse) => {
          should.exist(authResponse);
          authResponse.should.eql(true);
        });
    ...
    test("should not authorize Token with invalid scopes", () => {
      authService
        .authorizeToken(userAccessToken, "oauth2", ["otherScope", "someScope2"])
        .then((authResponse) => {
          should.exist(authResponse);
          authResponse.should.eql(false);
        });
