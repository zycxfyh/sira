...
describe('Auth tests', () => {
  ...
  describe('Credential Auth', () => {
    ...
    it('should authorize Credential with scopes', () => {
      return authService
        .authorizeCredential(userFromDb.id, 'oauth2', ['someScope1', 'someScope2'])
        .then(authResponse => {
          should.exist(authResponse);
    ...
    it('should not authorize Credential with invalid scopes', () => {
      return authService
        .authorizeCredential(userFromDb.id, 'oauth2', ['otherScope', 'someScope2'])
        .then(authResponse => {
          should.exist(authResponse);
          authResponse.should.eql(false);
        });
    ...
    it('should not authorize Credential that is inActive', () => {
      return credentialService
        ...
        .then(() =>
          authService.authorizeCredential(userFromDb.id, 'oauth2', ['otherScope', 'someScope2'])
        )
        .then(authResponse => {
          should.exist(authResponse);
          authResponse.should.eql(false);
          ...
          return credentialService.activateCredential(userFromDb.id, 'oauth2').then(res => {
            should.exist(res);
            res.should.eql(true);
          });
        });
  ...
  describe('Token Auth', () => {
    ...
    it('should authenticate token', () => {
      return authService.authenticateToken(userAccessToken, 'oauth2').then(authResponse => {
    ...
    it('should not authenticate invalid token', () => {
      authService.authenticateToken('invalidToken', 'oauth2').then(authResponse => {
        should.exist(authResponse);
        authResponse.should.eql(false);
      });
    });
