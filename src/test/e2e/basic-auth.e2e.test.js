...
describe('E2E: basic-auth Policy', () => {
  before('setup', () => {
    ...
    return cliHelper
      ...
      .then(newUser => {
        return cliHelper.runCLICommand({
          cliArgs: [
            'credentials create -t basic-auth -p "scopes=authorizedScope" -p "password=pass" -c ',
            username,
          ],
          adminPort,
          configDirectoryPath,
  ...
  it('should not authenticate token for requests without token header', () => {
    return request(`http://localhost:${gatewayPort}`).get('/authorizedPath').expect(401);
  });

  it("should not authenticate token for requests if requester doesn't have authorized scopes", () => {
  ...
  it('should authenticate token for requests with scopes if requester is authorized', () => {
    const credentials = Buffer.from(username.concat(':pass')).toString('base64');
