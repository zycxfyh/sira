...
const services = require('../../../src/core/services/index');
...
describe('Functional Tests oAuth2.0 Policy', () => {
    ...
    config.gatewayConfig = {
      ...
      pipelines: {
        pipeline1: {
          ...
          policies: [{ oauth2: {} }, { proxy: [{ action: { serviceEndpoint: 'backend' } }] }],
        },
        pipeline2: {
          ...
          .then(apps => {
            ...
            request
              ...
              .end((err, res) => {
                ...
                return serverHelper.generateBackendServer(6069).then(() => {
                  done();
                });
              });
          });
  ...
  it('should not authenticate token for requests without token header', done => {
    const request = session(app);

    request.get('/authorizedPath').expect(401).end(done);
  });

  it("should not authenticate token for requests if requester doesn't have authorized scopes", done => {
    const request = session(app);

    request.get('/unauthorizedPath').expect(401).end(done);
  });

  it('should authenticate token for requests with scopes if requester is authorized', done => {
    const request = session(app);
  ...
  it('should not authenticate invalid token', done => {
    const request = session(app);
