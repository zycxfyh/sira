  ...
  describe('Issues a JWT token when configured', () => {
    before(() => {
      return db
        ...
        .then(user =>
          credentialService.insertCredential(user.id, 'basic-auth', {}).then(cred => {
            Object.assign(userCred, { password: cred.password, username: user.username });
            return user;
          })
        )
        ...
        .then(credential =>
          credentialService
            .addScopesToCredential(credential.id, 'oauth2', ['read', 'write'])
            .then(() => credential)
        )
