...
const logger = require('../../../core/logger').policy;
...
describe('@proxy policy', () => {
  ...
  before('start HTTP server', done => {
    findOpenPortNumbers(1).then(ports => {
      ...
      expressApp.all('*', express.json(), (req, res) => {
  ...
  describe('proxyOptions', () => {
    ...
    describe('When proxy options are scattered on all the supported properties', () => {
      before(() => {
        return setupGateway(Object.assign(defaultProxyOptions, { proxyOptions: { xfwd: true } }), {
  ...
  describe('When proxy does not have the serviceEndpoint', () => {
    before(() => {
      return setupGateway(Object.assign(defaultProxyOptions, { serviceEndpoint: null })).then(
        apps => {
          app = apps.app;
        }
      );
              ...
              pipeline1: {
                ...
                policies: [
                  ...
                  },
                  {
                    proxy: [
                      {
                        action: Object.assign({}, defaultProxyOptions, {
                ...
                policies: [
                  {
                    proxy: [
                      {
                        action: Object.assign({ stripPath: true }, defaultProxyOptions, {
                          serviceEndpoint: 'backend',
                        }),
                      },
                    ],
