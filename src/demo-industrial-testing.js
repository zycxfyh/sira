...
class IndustrialTestingDemo {
  ...
  async demoReportGeneration() {
    ...
    try {
      ...
      console.log(
        `   测试摘要: ${report.summary.totalTests} 测试, 成功率 ${report.summary.successRate}`,
      );
    } catch (error) {
  ...
  showUsageExamples() {
    ...
    console.log(
      "./scripts/industrial-testing.sh --test-type comprehensive --coverage",
    );
    console.log("");
    console.log("# 查看测试报告");
  ...
  async showSystemInfo() {
    ...
    console.log(
      `进程内存使用: ${Math.round(process.memoryUsage().heapUsed / 1024 / 1024)} MB`,
    );
  }
}
