...
class ConversationManager extends EventEmitter {
  ...
  createSession(userId, options = {}) {
    ...
    log_info(`创建对话会话: ${session.id} - ${session.title}`);
  ...
  getStats() {
    ...
    return {
      ...
      avgMessagesPerSession: sessions.length > 0 ? Math.round(this.stats.totalMessages / sessions.length) : 0,
      avgTokensPerSession: sessions.length > 0 ? Math.round(this.stats.totalTokens / sessions.length) : 0
    };
  }
  ...
  cleanup() {
    ...
    if (cleanedCount > 0) {
      log_info(`清理了 ${cleanedCount} 个过期会话`);
    }
