...
class WebhookManager {
  ...
  getWebhookStats(webhookId = null) {
    if (webhookId) {
      ...
      const stats = {
        ...
        successRate: history.length > 0 ? (webhook.successCount / history.length * 100).toFixed(2) : 0,
  ...
  async processDeliveryQueue() {
    ...
    // 找到待处理的投递
    const pendingDelivery = this.deliveryQueue.find(delivery =>
      delivery.status === 'pending' &&
  ...
  async processDelivery(delivery) {
    ...
    try {
      ...
      if (delivery.attempts < maxRetries) {
        ...
        console.log(`🔄 Webhook重试安排: ${webhook.id}, ${delivery.attempts}/${maxRetries}, 延迟${delay}ms`)
      } else {
        delivery.status = 'failed'
    ...
    try {
      ...
      if (error.response) {
        ...
        if (error.response.status >= 400 && error.response.status < 500) {
          // 客户端错误，不重试
          throw new Error(`Webhook客户端错误: ${error.response.status} - ${error.response.statusText}`)
        } else {
          // 服务器错误，重试
          throw new Error(`Webhook服务器错误: ${error.response.status} - ${error.response.statusText}`)
        }
