...
class ParameterManager extends EventEmitter {
  ...
  validateParameters (parameters) {
    ...
    for (const [paramName, paramValue] of Object.entries(parameters)) {
      ...
      // 特殊检查
      if (paramName === 'top_p' && parameters.temperature && parameters.temperature > 0.5 && paramValue < 0.5) {
  ...
  analyzeParameterUsage (parameters, provider, model) {
    ...
    if (model && this.optimizationRules.modelOptimization[model]) {
      ...
      if (modelRules.max_tokens && parameters.max_tokens > modelRules.max_tokens) {
        analysis.suggestions.push(`建议将max_tokens降低到 ${modelRules.max_tokens} 以获得更好的性能`)
      }
    }
