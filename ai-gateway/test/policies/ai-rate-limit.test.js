...
const aiRateLimit = require('../../lib/policies/ai-rate-limit')

describe('AI Rate Limit Policy', function () {
  ...
  describe('Rate Limiting Logic', function () {
    it('should allow requests within limits', function (done) {
    ...
    it('should block requests exceeding limits', function (done) {
      const policy = aiRateLimit({
    ...
    it('should reset limits after window expires', function (done) {
      const policy = aiRateLimit({
  ...
  describe('Token Consumption', function () {
    it('should consume tokens based on request complexity', function () {
      const policy = aiRateLimit({
        windowMs: 60000,
        maxTokens: 100,
        tokensPerRequest: {
    ...
    it('should handle token calculation errors gracefully', function () {
      const policy = aiRateLimit({
        windowMs: 60000,
        maxTokens: 100,
  ...
  describe('Different Strategies', function () {
    it('should support IP-based rate limiting', function () {
      const policy = aiRateLimit({
        windowMs: 60000,
        maxTokens: 10,
        strategy: 'ip'
    ...
    it('should support user-based rate limiting', function () {
      const policy = aiRateLimit({
        windowMs: 60000,
        maxTokens: 10,
        strategy: 'user'
    ...
    it('should support API key-based rate limiting', function () {
      const policy = aiRateLimit({
        windowMs: 60000,
        maxTokens: 10,
        strategy: 'api-key'
  ...
  describe('Response Headers', function () {
    it('should set rate limit headers', function (done) {
      const policy = aiRateLimit({
        windowMs: 60000,
        maxTokens: 100,
        tokensPerRequest: 1
