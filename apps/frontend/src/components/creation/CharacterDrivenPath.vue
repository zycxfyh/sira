...
<template>
    <div id="creation-path-character" class="page active">
        <div class="center-content" style="justify-content: flex-start; padding-top: 2rem; text-align: left; align-items: stretch;">
          <h2>角色驱动路径：导入化身</h2>
          <p>上传一个预先定义好的角色卡（.json 格式）。世界架构AI将围绕您的角色特性和背景，量身定制一个与之相匹配的初始世界。</p>
          <div class="step-content" style="margin-top: 2rem; flex-grow: 1;">
            <label for="character-card-input">上传角色卡:</label>
            <input type="file" id="character-card-input" accept=".json" @change="handleCharacterCardUpload">
          ...
          <div class="button-group">
            ...
            <button class="button primary" :disabled="!appStore.uploadedCharacterCard" @click="onStartClick">载入化身并生成世界</button>
          </div>
        </div>
      </div>
</template>
