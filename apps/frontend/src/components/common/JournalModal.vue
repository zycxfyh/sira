  ...
  <div id="journal-modal" class="modal-backdrop" @click.self="uiStore.hideJournalModal">
    <div class="modal">
      ...
      <div class="modal-content">
        <div id="journal-window" style="background-color: #111; padding: 15px; border-radius: 5px; height: 60vh; overflow-y: auto;">
          <p 
            v-for="(p, index) in reversedNarrativeLog" 
            :key="index" 
            :style="{ fontStyle: p.isMeta ? 'italic' : 'normal', color: p.isMeta ? '#aaa' : 'var(--primary-text)' }"
          >
            {{ p.text }}
          </p>
        </div>
